...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".editBtn").forEach((button) => {
    button.addEventListener("click", function () {
      ...
      const productImages = this.dataset.images
        ? this.dataset.images.split(",")
        : [];
      const productOffer = this.dataset.offer
        ? JSON.parse(this.dataset.offer)
      ...
      document.getElementById("editProductCategory").value =
        productCategory || "";
      document.getElementById("editProductOffer").value =
    ...
    files.forEach((file, index) => {
      ...
      reader.onload = function (e) {
        ...
        img.onload = function () {
          ...
          buttons
            .querySelector(".save-crop")
            .addEventListener("click", function () {
              const croppedData = cropper
                .getCroppedCanvas()
...
document.addEventListener("DOMContentLoaded", function () {
  document.querySelectorAll(".toggle-offer").forEach((button) => {
    button.addEventListener("click", function () {
      ...
      axios
        .post(`/admin/toggle-offer/${productId}`, { isActive: newStatus })
        .then((response) => {
          if (response.data.success) {
            location.reload();
          }
...
async function deleteProduct(id) {
  try {
    ...
    if (response.ok) {
      showMessage(
        result.message || "Product unlisted successfully!",
        "success"
      );
